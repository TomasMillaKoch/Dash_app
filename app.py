...
prior_table = pd.read_csv(DATA_PATH.joinpath('priority_barriers.csv'), index_col=False)
inter_table = pd.read_csv(DATA_PATH.joinpath('inter_barriers.csv'), index_col=False)
...
app = Dash(__name__, title="WCRP DASHBOARD DRAFT - HORS")
server = app.server
cache = Cache()
...
prior_drop =  dcc.Dropdown(
                    options=[
                        {'label': 'Priority Barriers', 'value': 'priority'},
                        {'label': 'Intermediate Barriers', 'value': 'intermediate'}
                    ],
...
barrtype_filter_drop = dcc.Dropdown(
                        ...
                        {'label': 'Partial barriers', 'value': 'Partial'},
                        {'label': 'Full barriers', 'value': 'Full'},
                        {'label': 'Partial-full barriers', 'value': 'Partial-Full'},
                        {'label': 'Potential barriers', 'value': 'Potential'}
                        ],
...
pcsis_filter_drop = dcc.Dropdown(
                        ...
                        placeholder='Select PCSIS status...',
                        id= 'PCSIS',
                        style={'width': '300px'}
                    )
...
barrstat_filter_drop = dcc.Dropdown(
                        ...
                        placeholder='Select barrier status...',
                        id= 'barrstat',
                        style={'width': '300px'}
                    )
...
app.layout = dbc.Container([
    ...
    html.Div([   
        html.Header([
            html.Img(src="assets/cwf_logo.png", style={'width':'200px', 'display':'inline-block', 'margin-inline-start':'0.67em', 'margin-top':'0.67em'}),
            html.H1("WCRP Dashboard", id = "title", style={'display':'inline-block', 'margin-inline-start':'0.67em', 'margin-top':'-0.30em', 'color':'#4C4D4F'})
        ], style={"width":"100%"}, id="headrow"),
    ]),
    ...
    dbc.Row([
        ...
        html.Div([
            ...
            html.Div([
                html.Div('Barrier List:'),
                dbc.Col(prior_drop, width = 2, style={'display':'inline-block'})
            ], id="dddrop"),

            html.Div([
                html.Div('Priority Watershed:'),
                dbc.Col(watershed_drop, width = 2, style={'display':'inline-block'})
            ], id="dropdown", style={'display':'inline-block', 'margin-inline-start':'0.67em'}),

        #Priority Barrier List Filters
            html.Div([
                html.Div('Barrier Type:', style={'margin-inline-start':'0.67em'}),
                dbc.Col(barrtype_filter_drop, width = 2, style={'display':'inline-block', 'margin-inline-start':'0.67em'})
            ], id="barrdrop"),

            html.Div([
                html.Div('Next Steps:', style={'margin-inline-start':'0.67em'}),
                dbc.Col(nextstep_filter_drop, width = 2, style={'display':'inline-block', 'margin-inline-start':'0.67em'})
            ], id="extradropdown"),

            #Intermediate & All Barrier List Filters
            html.Div([
                html.Div('PCSIS Filter:', style={'margin-inline-start':'0.67em'}),
                dbc.Col(pcsis_filter_drop, width = 2, style={'display':'inline-block', 'margin-inline-start':'0.67em'})
            ], id="pcsisdrop"),

            html.Div([
                html.Div('Barrier Status:', style={'display':'inline-block', 'margin-inline-start':'0.67em'}),
                dbc.Col(barrstat_filter_drop, width = 2, style={'display':'inline-block', 'margin-inline-start':'0.67em'})
            ], id="extradropdown2"),
    ...
    dbc.Row([  
        dbc.Col([
            html.Div([
                    dl.Map(children=[
                    dl.LayersControl(
                    [dl.BaseLayer(dl.TileLayer(url='https://tiles.stadiamaps.com/tiles/alidade_smooth/{z}/{x}/{y}{r}.png',
                                attribution='&copy; <a href="https://stadiamaps.com/">Stadia Maps</a>'), name='ESRI Topographic', checked=True),
                                dl.BaseLayer(dl.TileLayer(), name='Base', checked=False)] +
                    ...
                    id='map',
                    style={'width': '100%', 'height': '500px'}, #style is key as map will not show up without it
                    center=[52.6,-120.5],
                    zoom=9 
                    ),
            ...
            html.Div([
                    dash_table.DataTable(
                        columns=[
                            ...
                            {'name': 'Access Model', 'id': 'access_model_ch_co_sk', 'type': 'text'},
                        ...
                        style_data={
                            ...
                            'text-align':'center',
                            'height':'auto',
                            #'whiteSpace':'normal' #Required for cell values to wrap
                        },
                        style_header={
                            'backgroundColor': '#43666b',
                            'fontWeight': 'bold',
                            'text-align':'center'
                        },
                        style_cell={
                        ...
                        page_action='none',
                        style_table={'height': '500px', 'maxHeight':'500px'}, #both height and max height required to set table height
                        fixed_rows={'headers':True,'data':0},
                        fill_width=True, 
                        id='table2',
                        active_cell= None
                    ),
            ], style={'width':'50%','margin-inline-start':'0.67em','display': 'inline-block'}),
        ], style={'display':'flex'}),
    ]),
...
@app.callback(
    [Output('barrdrop','style'),Output('extradropdown','style'),Output('pcsisdrop','style'),Output('extradropdown2','style')], [Input('dd','value')]
)

def show_filter(value):
    if value == 'priority':
